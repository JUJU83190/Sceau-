...
const SUFFIXES_GENERIQUES = new Set(["ltd","llc","inc","sa","sas","sarl","gmbh","corp","co","com","net","org","io","fr","eu","group","holdings","limited","capital","invest","investments","trading","partners","financial","bank","sgp"]);

function tokenize(str){ return str.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").split(/[^a-z0-9]+/).filter(Boolean); }
function coreTokens(str){ const all=tokenize(str); const s=all.filter(t=>!SUFFIXES_GENERIQUES.has(t)&&t.length>1); return s.length?s:all; }

// Normalisation stricte : garde tout sauf casse/accents/ponctuation
function strictNormalize(str){ return str.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]/g,""); }
// Normalisation "coeur" : uniquement les tokens significatifs concaténés (ignore suffixes génériques)
function coreNormalize(str){ return coreTokens(str).join(""); }
